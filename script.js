...
if (typewriterElement) {
    typewriterElement.textContent = typewriterElement.dataset.typewriter || typewriterElement.textContent;
}
...
if (bubbleArea && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    ...
    const createBubble = () => {
        ...
        const size = Math.round(randomBetween(30, 44));
        const drift = Math.round(randomBetween(-24, 24));
        const rotate = Math.round(randomBetween(-7, 7));
        const duration = randomBetween(2.67, 4).toFixed(2);

        bubble.className = "service-bubble";
        bubble.textContent = pickLabel();
        const side = Math.random() > 0.5 ? randomBetween(72, 96) : randomBetween(4, 28);

        bubble.style.setProperty("--bubble-left", `${side.toFixed(1)}%`);
        bubble.style.setProperty("--bubble-top", `${randomBetween(72, 96).toFixed(1)}%`);
    ...
    window.setInterval(() => {
        ...
        if (Math.random() > 0.82) {
            window.setTimeout(createBubble, 200);
        }
    }, 600);
}
